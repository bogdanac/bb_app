...
// Tomorrow at 8:00 PM
    ...
    test('reminder uses 7-day scheduling for reliability', () {
      // GIVEN: Food tracking notifications scheduled for 7 days ahead
      // Instead of using DateTimeComponents.time (which can be unreliable on Android),
      // we schedule individual notifications for each of the next 7 days
      const daysScheduled = 7;

      // THEN: 7 days of notifications should be scheduled
      expect(daysScheduled, equals(7),
          reason: 'Food tracking reminders must schedule 7 days ahead for reliability');
    });

    test('food tracking reminder uses correct notification ID range', () {
      // GIVEN: Food tracking reminder notification IDs (7770-7779 for 7-day scheduling)
      const foodTrackingNotificationIdStart = 7770;
      const foodTrackingNotificationIdEnd = 7779;

      // WHEN: We verify the IDs are in the correct range
      // Food tracking uses 7770-7779 (7 days of reminders)
      // Water uses 1-4, fasting uses 100-120, tasks use 1000-9999, routines use 2000-9999
      // Note: 7770-7779 is within task range but uses unique IDs that won't be generated by hash-based task ID algorithm

      // THEN: ID range should not conflict with critical notification types
      expect(foodTrackingNotificationIdStart, isNot(inInclusiveRange(1, 4)),
          reason: 'Should not conflict with water notifications');
      expect(foodTrackingNotificationIdEnd, isNot(inInclusiveRange(100, 120)),
          reason: 'Should not conflict with fasting notifications');
      expect(foodTrackingNotificationIdStart, equals(7770),
          reason: 'Must start at ID 7770 for 7-day scheduling');
      expect(foodTrackingNotificationIdEnd - foodTrackingNotificationIdStart, greaterThanOrEqualTo(6),
          reason: 'Must have at least 7 IDs for 7-day scheduling');
    });

    test('scheduling food tracking reminder cancels previous reminders', () {
      // GIVEN: Existing food tracking reminders (7770-7779 for 7-day scheduling)
      final existingReminderIdStart = 7770;
      final existingReminderIdEnd = 7779;
      final legacyReminderId1 = 7777; // Legacy ID
      final legacyReminderId2 = 7776; // Legacy ID

      // WHEN: We reschedule the food tracking reminder
      // The implementation should cancel all IDs (7770-7779 + legacy) before scheduling new ones

      // THEN: All IDs should be in expected range
      expect(existingReminderIdStart, equals(7770));
      expect(existingReminderIdEnd, equals(7779));
      expect(legacyReminderId1, equals(7777));
      expect(legacyReminderId2, equals(7776));
    });
