...
class _ActivityDetailScreenState extends State<ActivityDetailScreen> {
  ...
  Future<void> _addManualTime({DateTime? initialDate}) async {
    final dateToUse = initialDate ?? _selectedDate ?? DateTime.now();
    final result = await AddManualTimeDialog.show(
      context,
      activityId: widget.activity.id,
      onAdded: () {}, // Will handle refresh below
    ...
    if (result == true && mounted) {
      // Reload history
      await _loadHistory();
      // Set the selected date to the added date
      setState(() {
        _selectedDate = dateToUse;
      });
      // Load sessions for that day
      await _loadSelectedDaySessions();
      // Show feedback
      if (mounted) {
        SnackBarUtils.showSuccess(context, 'Time added successfully');
      }
    }
  }
