...
class ScheduledFastingsService {
  ...
  /// Generate schedule for a specific number of months ahead
  static Future<List<ScheduledFasting>> generateScheduleForMonths(int months) async {
    final List<ScheduledFasting> schedule = [];
    final now = DateTime.now();
    ...
    // Get preferred fasting days from settings
    final prefs = await SharedPreferences.getInstance();
    final preferredDay = prefs.getInt('preferred_fasting_day') ?? 5;
    final preferredMonthlyDay = prefs.getInt('preferred_monthly_fasting_day') ?? 25;
    final useMenstrualScheduling = prefs.getBool('fasting_use_menstrual_scheduling') ?? false;

    // Keep manually created fastings
    final existingFastings = await getScheduledFastings();
    final manualFastings = existingFastings.where((f) => !f.isAutoGenerated).toList();

    // If menstrual scheduling is enabled, use Dr. Pelz's cycle-based protocol
    if (useMenstrualScheduling) {
      final cycleSchedule = await _generateCycleBasedScheduleForMonths(prefs, now, endDate, preferredDay);
      // Combine manual and auto-generated
      final combined = [...manualFastings, ...cycleSchedule];
      combined.sort((a, b) => a.date.compareTo(b.date));
      await saveScheduledFastings(combined);
      return combined;
    }

    // Fixed-day scheduling
    DateTime current = DateTime(now.year, now.month, now.day - 3);
    DateTime? lastFastEnd;
    ...
    // Combine manual and auto-generated
    final combined = [...manualFastings, ...schedule];
    combined.sort((a, b) => a.date.compareTo(b.date));
    await saveScheduledFastings(combined);
    return combined;
  }

  /// Generate cycle-based schedule for a specific number of months
  static Future<List<ScheduledFasting>> _generateCycleBasedScheduleForMonths(
    SharedPreferences prefs,
    DateTime now,
    DateTime endDate,
    int preferredWeekday,
    ...
    final lastPeriodStartStr = prefs.getString('last_period_start');
    final averageCycleLength = prefs.getInt('average_cycle_length') ?? 28;
    final longFastCycleDay = prefs.getInt('fasting_long_fast_cycle_day') ?? 7;
    ...
    for (int i = 0; i < cycleStarts.length; i++) {
      final cycleStart = cycleStarts[i];
      final longFastDate = cycleStart.add(Duration(days: longFastCycleDay - 1));
    ...
    // Schedule weekly fasts on safe days
    DateTime current = startDate;
    while (current.isBefore(endDate)) {
      if (current.weekday == preferredWeekday) {
        final cycleDay = _getCycleDayForDate(current, cycleStarts, averageCycleLength);
